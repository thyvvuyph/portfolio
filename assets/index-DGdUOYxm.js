...
`;document.querySelectorAll('a[href^="#"]').forEach(r=>{r.addEventListener("click",function(s){s.preventDefault(),document.querySelector(this.getAttribute("href")).scrollIntoView({behavior:"smooth"})})});let i=0;const o=[{num:"01/04",title:"Designing a Social Media App for the Black Experience",type:"Branding Project • Web Design • Mobile Apps"},{num:"02/04",title:"E-commerce Platform Redesign",type:"UI/UX Design • Web Design"},{num:"03/04",title:"Brand Identity for Tech Startup",type:"Branding Project • Digital Marketing"},{num:"04/04",title:"Mobile Banking App Interface",type:"Mobile Apps • UI/UX Design"}];window.moveProject=r=>{i=(i+r+o.length)%o.length;const s=o[i];document.getElementById("project-container").innerHTML=`
    <span class="text-gray-500 text-sm">${s.num}</span>
    <h4 class="text-xs tracking-widest text-gray-500 mt-8">PROJECT</h4>
    <h3 class="text-3xl font-bold my-4">${s.title}</h3>
    <h4 class="text-xs tracking-widest text-gray-500 mt-8">TYPE</h4>
    <p class="text-gray-600 mt-2">${s.type}</p>
  `};
