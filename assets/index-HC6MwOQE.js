    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ${t.works.projects.map((e,n)=>`
        <div class="p-8 border-2 border-white/30 rounded-2xl bg-white/10 hover:bg-white/20 transition flex flex-col gap-4">
          <span class="text-white/50 text-xs">0${n+1}/${t.works.projects.length.toString().padStart(2,"0")}</span>
...
`,document.querySelectorAll('a[href^="#"]').forEach(e=>{e.addEventListener("click",function(n){n.preventDefault(),document.querySelector(this.getAttribute("href")).scrollIntoView({behavior:"smooth"})})})}d();let s=0;window.toggleLang=()=>{a=a==="en"?"vi":"en",s=0,d()};window.moveProject=t=>{const e=c[a];s=(s+t+e.works.projects.length)%e.works.projects.length;const n=e.works.projects[s];document.getElementById("project-container").innerHTML=`
    <span class="text-white/60 text-sm">0${s+1}/04</span>
